#!/usr/bin/env node
/**
 * dsh-session-quality-xg 构建脚本（跨平台 Node，无外部依赖）。
 *
 * 步骤：
...
if (!bundle.includes('id: "dsh-session-quality-xg"')) {
  throw new Error('client bundle 缺少插件 id')
}
if (!bundle.includes('conversation.view') || !bundle.includes('session-quality/api')) {
